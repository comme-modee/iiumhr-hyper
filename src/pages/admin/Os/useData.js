...
export default function useData() {
    ...
    const getData = async ( type, selectedDate ) => {
        ...
        try {
            const res = await ( type === 'day' ? osApi.osDay(formatDate) :
                                type === 'week' ? osApi.osWeek(formatDate) :
                                type === 'month' ? osApi.osMonth(formatDate) :
                                null )
    ...
    const labels = data.map(i => i.operatingSystem || '알수없음');
